...
namespace MasterMechData
{
    public class User
    {
        ...
        [Required]
        [DataType(DataType.Password)] // using this, at run time input Password is in star format
        ...
        public bool ValidLogin(string isConStr)
        {
           ...
           //msPassword = MasterMechUtil.Decrypt(msPassword);
            using (SqlConnection lObjCon = new SqlConnection(isConStr))
            {

                string lsQuery = "SELECT * FROM UserDtl WHERE UserID = @UserID AND Pwd=@Pwd AND Deleted='N'";
                try
                {
                        ...
                        using (SqlDataReader lObjSDR = cmd.ExecuteReader())
                        {
                            if (lObjSDR.HasRows)
                            {
                                while (lObjSDR.Read())
                                {
                                    ...
                                    MasterMechUtil.msUserName = lObjSDR["UserName"].ToString();
                                    lbValidUser = true;
                                }
                            }
                        }
        ...
        public User SearchSingleRecord(string isUserID)
        {
           
            string query = $"SELECT * FROM UserDtl Where UserId =  '{isUserID}' and Deleted = 'N'";
                ...
                using (SqlCommand command = new SqlCommand(query, connection))
                {
                    ...
                    using (SqlDataReader reader = command.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            ...
                            msUserID = reader["UserID"].ToString();
                            msPassword = reader["Pwd"].ToString();

                            msUserName = reader["UserName"].ToString();
                            msMobileNo = reader["MobNo"].ToString();
                            msEmail = reader["EmailID"].ToString();
                            msUserType = reader["UserType"].ToString();
                            msRemarks = reader["Remarks"].ToString();


                            mdLastLoginTime = reader["LastLoginTime"] != DBNull.Value ? (DateTime)reader["LastLoginTime"] : (DateTime?)null;

                            mdLastPasswordChangeTime = reader["LastPwdChangeTime"] != DBNull.Value ? (DateTime)reader["LastPwdChangeTime"] : (DateTime?)null;


                            msCreatedBy = reader["CreatedBy"].ToString();
                            mdCreated = (DateTime)reader["Created"];

                            mdModified = reader["Modified"] != DBNull.Value ? (DateTime)reader["Modified"] : (DateTime?)null;
        ...
        public void DeleteData(string isUserID)
        {
            try
            {
                ...
                //Following Lines are for window app only
        ...
        public bool SaveSQL(MasterMechUtil.OPMode inMode)
        {
            ...
            using (SqlConnection lObjConn = new SqlConnection(msConnStr))
            {
                if (inMode == MasterMechUtil.OPMode.New)
                {
                    ...
                    lsQueryText += "(@UserID, @Pwd, @UserName, @MobNo, @EmailID, @UserType, NULL, NULL, @Remarks, @Created, @CreatedBy, NULL, NULL, 'N', NULL, 'N')";
                    lbOpType =  true;
                }
                else
                {
